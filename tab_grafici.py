# -*- coding: utf-8 -*-
"""Mixin per il Tab Grafici (Lamborghini/Persico/Cannon PDF generator).
...
# ── costanti locali (re-export da pdf_export per comodità) ──────────
...
_SERIES_COLORS = {
    "T1":"#ff6b6b",  "T2":"#ff922b",  "T3":"#ffd43b",  "T4":"#a9e34b",
    "TS1":"#ff6b6b", "TS2":"#ff922b", "TS3":"#ffd43b", "TS4":"#a9e34b",
    "TS5":"#69db7c", "TS6":"#38d9a9", "TS7":"#4dabf7", "TS8":"#748ffc",
    "TI1":"#da77f2","TI2":"#f783ac","TI3":"#ff8787","TI4":"#ffa94d",
    "P1":"#74c0fc",  "P2":"#a9e34b",  "P3":"#63e6be",
    "Z1":"#74c0fc",  "Z2":"#a9e34b",
    "FC":"#6c63ff",  "SPC":"#00d4ff", "Forza":"#6c63ff",
    "F1":"#6c63ff",  "F2":"#00d4ff",  "F3":"#52c41a",  "F4":"#faad14",
    "VS1":"#a9e34b", "VS2":"#63e6be", "V1":"#a9e34b",  "V2":"#63e6be",
    "VI1":"#74c0fc", "VI2":"#748ffc", "VI3":"#da77f2", "VI4":"#f783ac",
    "V3":"#74c0fc",  "V4":"#748ffc",
}


def _pdf_parse_csv(path):
    """Wrapper locale per parse CSV usato nel tab Grafici."""
    from pdf_export import _parse_csv_for_pdf
    return _parse_csv_for_pdf(path)


def _parse_native_pack(native_paths, stampo):
    from pdf_export import _parse_native_pack as _pnp
    return _pnp(native_paths, stampo)


def _merge_native_data(data, meta, nd, mp):
    from pdf_export import _merge_native_data as _mnd
    return _mnd(data, meta, nd, mp)


def _filter_partite(partite, stampo):
    from pdf_export import _filter_partite as _fp
    return _fp(partite, stampo)


class GraficiMixin:
    """Mixin che aggiunge il Tab Grafici a LogAnalyzerApp."""
    ...
    # ------------------------------------------------------------------
    # Fullscreen con hover interattivo (blit)
    # ------------------------------------------------------------------
    def _grafici_open_fullscreen(self, key):
        ...
        _MC = {"active_bg": "#6c63ff", "active_fg": "#ffffff",
        ...
        def _set_mode(m):
            ...
            for _m, _b in _mode_btns.items():
                _b.configure(bg=_MC["active_bg" if _m == m else "idle_bg"],
                             fg=_MC["active_fg" if _m == m else "idle_fg"])
            _clear_overlay()

        for lbl in ["Nessuno", "Tooltip", "Crosshair", "Snap", "Completo"]:
            _l = lbl
            b = tk.Button(mode_bar, text=lbl, bg=_MC["idle_bg"], fg=_MC["idle_fg"],
            ...
            _mode_btns[lbl] = b
        _mode_btns["Nessuno"].configure(bg=_MC["active_bg"], fg=_MC["active_fg"])
        ...
        def _style_toolbar(widget):
            ...
            for child in widget.winfo_children(): _style_toolbar(child)

        toolbar.update(); _style_toolbar(toolbar)
        canvas.get_tk_widget().pack(fill="both", expand=True)
        canvas.draw()

        _bg = [None]

        def _save_bg(): _bg[0] = canvas.copy_from_bbox(fig.bbox)
        ...
        _tooltip = ax.annotate("", xy=(0, 0), xytext=(15, 15), textcoords="offset points",
        ...
        def _place_tooltip(bx, by):
            ...
            _tooltip.xyann = (-120 if xfrac > 0.75 else 15, -60 if yfrac > 0.70 else 15)

        def _clear_overlay():
            _vline.set_data([], []); _hline.set_data([], [])
            _snap_dot.set_data([], [])
            _tooltip.set_visible(False); _info_box.set_visible(False)
            if _bg[0] is not None:
                canvas.restore_region(_bg[0]); canvas.blit(fig.bbox)
            ...
            if mode == "Tooltip":
                ...
                for param, (xs_arr, ys_arr, _c) in _series.items():
                    ...
                    if nx is not None and dist < best_dist:
                        best_dist = dist; best_param = param; best_x = nx; best_y = ny
                if best_param is not None and best_dist < 40:
                    _tooltip.xy = (best_x, best_y); _place_tooltip(best_x, best_y)
                    _tooltip.set_text(f"{best_param}\nX: {best_x:.1f} s\nY: {best_y:.4g}")
                    _tooltip.set_visible(True)
                else: _tooltip.set_visible(False)
                ...
                best_param, best_x, best_y, best_dist, best_color = None, None, None, float("inf"), "#fff"
        ...
        canvas.mpl_connect("axes_leave_event", lambda e: _clear_overlay())
        ...
        def _on_scroll(event):
            ...
            if new_xl[1] > new_xl[0] and new_yl[1] > new_yl[0]:
                ax.set_xlim(new_xl); ax.set_ylim(new_yl); canvas.draw(); _save_bg()

        _clamping = [False]

        def _on_xlim_change(ax_c):
            if _clamping[0]: return
            xl = list(ax_c.get_xlim())
            if xl[0] < _orig_xlim[0]:
                _clamping[0] = True
                ax_c.set_xlim(_orig_xlim[0], xl[1] + (_orig_xlim[0] - xl[0]))
                _clamping[0] = False

        def _on_ylim_change(ax_c):
            if _clamping[0]: return
            yl = list(ax_c.get_ylim())
            if yl[0] < _orig_ylim[0]:
                _clamping[0] = True
                ax_c.set_ylim(_orig_ylim[0], yl[1] + (_orig_ylim[0] - yl[0]))
                _clamping[0] = False
    ...
    def _grafici_open_csv(self):
        ...
        try:
            ...
            if self._grafici_native_paths:
                try:
                    nd, mp = _parse_native_pack(self._grafici_native_paths,
                                                self._grafici_stampo_var.get())
    ...
    def _grafici_open_native(self):
        _TYPES = {"FORZA": "FORZA", "POSIZIONE": "POSIZIONE",
        ...
        if n == 0:
            self._grafici_native_lbl.configure(
                text="Nessun file riconosciuto", text_color=COLORS["warn"]); return
    ...
    def _grafici_open_logo(self):
        ...
        self._grafici_logo_lbl.configure(text=os.path.basename(path),
                                          text_color=COLORS["text"])
    ...
    def _grafici_generate(self):
        ...
        _base = (os.path.splitext(os.path.basename(self._grafici_csv_path))[0]
                 if self._grafici_csv_path else f"Grafico-{stampo.replace(' ', '_')}")
        out = filedialog.asksaveasfilename(
            defaultextension=".pdf", initialfile=f"{_base}.pdf",
            filetypes=[("PDF", "*.pdf")])
        if not out: return
        ...
        note_text = (self._grafici_note_box.get("1.0", "end").strip()
    ...
    def _grafici_done(self, path):
        ...
        self._grafici_status.configure(
            text=f"OK PDF salvato: {os.path.basename(path)}", text_color=COLORS["info"])
