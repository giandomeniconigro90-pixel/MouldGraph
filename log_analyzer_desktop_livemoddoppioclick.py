...
def _parse_long_cycle_csv(filepath):
    ...
    # header separato da virgola, dati da punto-e-virgola
    ...
    def _parse_ts(s):
        try:
            return datetime.strptime(s, _LONG_TS_FMT)
        except Exception:
            return None
    ...
    return rows, headers, col_meta, numeric_cols, datetime_cols, binary_cols, ";", detected_enc
...
class InteractivePlotCanvas:
    ...
    def _on_release(self, event):
        if getattr(self, "_pan_moved", False):  # A1: draw solo se pan reale
            self._drawing = True
            try:
                self._ax1.autoscale(False, axis="y")
                if self._ax2:
                    self._ax2.autoscale(False, axis="y")
                self._canvas.draw()
                self._bg_cache = self._canvas.copy_from_bbox(self._fig.bbox)
            finally:
                self._drawing = False
    ...
    def _redraw_cursors_ab(self):
        ...
        if self._cur_a is not None:
            line_a, = ax1.plot(
                [self._cur_a, self._cur_a], list(ylim),
                color=COLORS["accent"], linewidth=1.2,
                linestyle="--", zorder=9)
            txt_a = ax1.text(
                self._cur_a, ylim[1], " A",
        ...
        if self._cur_b is not None:
            line_b, = ax1.plot(
                [self._cur_b, self._cur_b], list(ylim),
                color=COLORS["accent2"], linewidth=1.2,
                linestyle="--", zorder=9)
            txt_b = ax1.text(
                self._cur_b, ylim[1], " B",
    ...
    def _do_pan(self, event):
        if event.x is None or self._pan_xlim is None:
            return
...
class LogAnalyzerApp(ctk.CTk):
    ...
    def _csv_open_compare(self):
        ...
        def run():
            try:
                ...
                self._csv_col_meta2 = col_meta2  # salva col_meta del 2° CSV
                self._csv_num_cols2 = list(num_cols2)  # colonne numeriche del 2° CSV
                self._csv_filepath2 = path
                # colori distinti (sfumati) per il secondo CSV
                palette2 = ["#ff6b6b","#ffd93d","#6bcb77","#4d96ff",
                            "#c77dff","#f9844a","#90e0ef","#f15bb5"]
                for i, col in enumerate(num_cols2):
                    self._csv_colors2[col] = palette2[i % len(palette2)]
                self.after(0, self._csv_rebuild_series2_panel)
                self.after(0, self._csv_refresh_plot)
    ...
    def _csv_open(self):
        ...
        def run():
            try:
                ...
                self.after(0, self._csv_populate_controls)
                self.after(0, lambda: self.set_status(
    ...
    def _live_toggle_multi(self):
        ...
        if self._live_multi_mode:
            ...
            if self._live_running or self._live_paused:
                self._live_stop()
            ...
            for col_i in range(2):
                ...
                def _make_ctrl(st=state, pb=play_btn, psb=pause_btn):
                    ...
                    def _stop():
                        ...
                        if all_stopped and self._live_running:
                            self._live_stop()
    ...
    def _live_open_csv(self):
        ...
        try:
            if _is_long_cycle_csv(path):
                rows, headers, col_meta, numeric_cols, _, _, _, _ = \
                    _parse_long_cycle_csv(path)
                self._live_log_append("Formato rilevato: Long CSV (Cannon/Persico/Krauss Maffei)")
            else:
                rows, headers, col_meta, numeric_cols, _, _, _, _ = \
                    parse_universal_csv(path)
            ...
            self._live_log_append(
                f"Colonne numeriche: {len(numeric_cols)} — {', '.join(numeric_cols[:12])}")
        except Exception as e:
            self._live_log_append(f"Errore lettura CSV: {e}")
    ...
    def _live_clear(self):
        _was_running = self._live_running
        if _was_running:
            self._live_running = False
        self._live_stop_interp_timer()
        self._live_rows.clear()
    ...
    def _live_sim_thread(self):
        ...
        def _wait_if_paused():
            ...
            while self._live_paused and self._live_running:
                time.sleep(0.1)
        ...
        try:
            if _is_long_cycle_csv(self._live_csv_path):
                ...
                self._live_log_append("Formato: Long CSV (Cannon/Persico/Krauss Maffei)")
            else:
                rows, headers, col_meta, numeric_cols, datetime_cols, _, _, _ = \
    ...
    def _live_serial_thread(self, port, baud):
        ...
        try:
            ...
            self._live_log_append(f"Seriale aperta: {port} @ {baud}")
        except Exception as e:
    ...
    def _live_poll(self):
        if not self._live_running:
            return
        drained = 0
        while drained < 500:  # aumentato da 200 a 500
            try:
    ...
    def _live_refresh_plot(self):
        ...
        if current_rules != getattr(self, "_live_last_rules_snapshot", None):
            ...
            self._live_ipc._needs_full_redraw = True
        self._live_ipc.update_live(xs_raw, series, x_type,
                                   self._live_col_meta, colors=colors)
    ...
    def _live_interp_tick(self):
        ...
        if not self._live_running or not self._live_rows or len(self._live_rows) < 2:
    ...
    def _live_export_csv(self):
        ...
        if not path:
            return
        # raccoglie tutte le chiavi non private
        all_keys = []
