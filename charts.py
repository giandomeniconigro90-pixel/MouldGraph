...
"""
charts.py
Widget canvas tkinter per visualizzazione dati:
  - TimelineCanvas  : barchart log per livello/ora
  - LineChart       : grafico lineare Siemens CSV
  - PumpCanvas      : stato pompa ON/OFF nel tempo
  - _isolate_scroll : helper isolamento scroll mouse
"""
...
class TimelineCanvas(tk.Canvas):
    """Barchart impilato per eventi log raggruppati per minuto."""
    ...
    def redraw(self):
        ...
        for i, key in enumerate(keys):
            ...
            sy = h - pb
            for lvl, color in [("DEBUG", "#1890ff"), ("INFO", "#52c41a"),
        ...
        # legenda
        lx = pl
        for lvl, color in [("ERROR", "#ff4d4f"), ("WARN", "#faad14"),
...
class LineChart(tk.Canvas):
    """Grafico lineare per dati Siemens CSV. series = {label: (color, [values])}"""
        ...
        for label, (color, vals) in self.series.items():
            ...
            if pts:
                self.create_oval(pts[-1][0] - 3, pts[-1][1] - 3,
                                 pts[-1][0] + 3, pts[-1][1] + 3, fill=color, outline="")
        # legenda
        lx, ly = pl, 5
...
class PumpCanvas(tk.Canvas):
    """Barchart orizzontale per stato pompa ON/OFF nel tempo."""
...
def _isolate_scroll(scrollable_frame):
    ...
    è dentro un CTkScrollableFrame (fix Windows/CustomTkinter)."""
    sf     = scrollable_frame
    ...
    sf.bind("<Enter>", lambda e: sf.bind_all("<MouseWheel>", _block))
    sf.bind("<Leave>", lambda e: sf.unbind_all("<MouseWheel>"))
